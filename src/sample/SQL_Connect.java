...
public class SQL_Connect {
    ...
    public void Add_OneData(String STOCKID, String STOCKNAME, String PRICE, String NUM, int in_out_mode) throws Exception {
        ...
        String query = "";
        float handling_fee = Float.parseFloat(Calculate.calHandlingFee(Calculate.mul(PRICE, NUM)));
        float tax = 0;
        tax = (in_out_mode == 1) ? Float.parseFloat(Calculate.calTax(Calculate.mul(PRICE, NUM))) : 0;
        float sum= (in_out_mode == 1) ?
                Float.parseFloat(Calculate.minus(Calculate.mul(PRICE, "1000"),
                Calculate.add(String.valueOf(tax), String.valueOf(handling_fee)))) :

                Float.parseFloat(Calculate.add(Calculate.mul(PRICE, "1000"),
                        Calculate.add(String.valueOf(tax), String.valueOf(handling_fee))));
        ...
        //含稅率進階資料表
        query = "INSERT INTO cost_tax_table(ID,Handling_fee,TAX,SUM) VALUES(" +
                ID + "," +
                handling_fee + "," +
                tax + "," +
                sum + ");";
